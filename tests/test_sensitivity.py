...
class TestVaryClusteringAlgorithm:
    def test_runs_multiple_algorithms(self, well_separated_data):
        result = vary_clustering_algorithm(
            well_separated_data, n_clusters=3, seed=42
        )
    ...
    def test_reproducibility(self, well_separated_data):
        r1 = vary_clustering_algorithm(
            well_separated_data, n_clusters=3, seed=42
        )
        r2 = vary_clustering_algorithm(
            well_separated_data, n_clusters=3, seed=42
        )
...
class TestVaryNClusters:
    def test_basic_operation(self, well_separated_data):
        result = vary_n_clusters(
            well_separated_data, cluster_range=(2, 5), seed=42
        )
    ...
    def test_adjacent_k_higher_ari(self, well_separated_data):
        result = vary_n_clusters(
            well_separated_data, cluster_range=(2, 6), seed=42
        )
    ...
    def test_config_names(self, well_separated_data):
        result = vary_n_clusters(
            well_separated_data, cluster_range=(2, 4), seed=42
        )
...
class TestRunSensitivityAnalysis:
    ...
    def test_most_least_sensitive(self, well_separated_data):
        ...
        assert result.most_sensitive_parameter in [
            p.value for p in SensitivityParameter
        ]
        assert result.least_sensitive_parameter in [
            p.value for p in SensitivityParameter
        ]
