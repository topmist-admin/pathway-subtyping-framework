#!/usr/bin/env python3
"""
Run pathway-subtyping framework on GEO benchmark datasets.

This version is optimized for GEO datasets and includes fixes for gene selection
with non-standard data formats.

Generates bootstrap_threshold_calibration_47datasets_geo.csv with:
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('benchmark_run_geo.log'),
...
RESULTS_CSV = RESULTS_DIR / 'bootstrap_threshold_calibration_47datasets_geo.csv'
...
def select_top_genes(gene_expression: np.ndarray, n_genes: int = 1000) -> np.ndarray:
    ...
    Robust version that handles both numpy arrays and converts to proper format.
    ...
    Returns:
        Filtered gene expression array (n_samples, n_genes_selected)
    """
    ...
    try:
        ...
        # Get top gene indices - ensure this returns integer array
        top_indices = np.argsort(gene_vars)[-n_genes:]

        # Ensure top_indices is numpy array of integers
        if not isinstance(top_indices, np.ndarray):
            top_indices = np.asarray(top_indices, dtype=int)

        # Select genes - convert to array to handle fancy indexing robustly
        selected = np.asarray(gene_expression[:, top_indices])

        logger.debug(f"        Selected {n_genes} genes (variance-based) from {gene_expression.shape[1]}")
        return selected

    except Exception as e:
        logger.warning(f"        Gene selection failed ({e}), using all genes")
        return gene_expression
...
def run_single_benchmark(
    ...
    try:
        # Ensure input is numpy array
        if not isinstance(gene_expression, np.ndarray):
            gene_expression = np.asarray(gene_expression, dtype=np.float64)
...
def main():
    """Main benchmark runner for GEO datasets."""
    import argparse

    parser = argparse.ArgumentParser(description='Run pathway-subtyping benchmark on GEO datasets')
    ...
    logger.info("=" * 80)
    logger.info("PATHWAY-SUBTYPING GEO-BENCHMARK CALIBRATION RUN")
    if args.test:
    ...
    # Filter to GEO datasets only
    manifest_df = manifest_df[manifest_df['source'] == 'GEO']
    logger.info(f"Filtered to GEO source: {len(manifest_df)} dataset(s)")
