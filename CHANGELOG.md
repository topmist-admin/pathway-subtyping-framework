...
#### BPU Molecular Optimization (`[bpu]` extra)
- **BPUGeneSetRegistry**: 18 curated neural pathway gene sets across 7 categories (learning, memory, E/I balance, reward, energy, maturation, olfactory) sourced from KEGG, Reactome, GO, Allen Brain Atlas
- **PerformanceCorrelator**: Pathway-to-performance correlation engine with Spearman + bootstrap CI, build cycle tracking, regression prediction, actionable recommendations with engineering hints
- **EnergyProfiler**: OXPHOS/glycolysis efficiency ratio, mTOR/AMPK sensing, per-cell-type metabolic cost estimation, cell composition recommendations for the 10 mWh/day constraint
- **MaturationScorer**: 6-stage organoid maturation classification (progenitor to mature neuron + glia), composite maturation index, learning readiness gate, days-to-readiness estimation
- **EIBalanceAnalyzer**: Excitatory/inhibitory ratio quantification with seizure/silence risk classification, per-cell resolution, cell composition adjustment recommendations
- **RewardProfiler**: Dopaminergic and serotonergic reward pathway profiling, D1/D2 receptor ratio, learning rate correlation
- **BuildComparator**: Cross-build comparison with composite ranking, trend analysis, HTML reports, and JSON export for Tribe lab workflow integration
- **`bpu_neural_pathways.gmt`**: 18 gene sets (~1,000 unique genes) bundled with the package

#### REST API (`[api]` extra)
- **FastAPI microservice** (`pathway_subtyping.api.app`) with 9 endpoints: `/status`, `/profile`, `/qc`, `/qc/energy`, `/qc/maturation`, `/qc/ei-balance`, `/correlate`, `/recommend`, `/compare`
- **Pydantic schemas** for all request/response models
- **API key authentication** via `PSF_API_KEY` environment variable and `X-API-Key` header
- Run with: `uvicorn pathway_subtyping.api.app:app --host 0.0.0.0 --port 8000`

#### L2/L3/L4 QC Validation Framework
- **DatasetRegistry**: YAML-based catalog of public datasets (TCGA-COAD, GSE65682, GSE15402) with per-feature applicability mappings
- **BulkQCAdapter**: Re-frames 9 manufacturing QC features (F1, F3, F6-F12) for bulk RNA-seq cohorts (per-sample scoring)
- **EffectSizeCalculator**: Spearman, AUROC, Kruskal-Wallis, Mann-Whitney, ICC reproducibility with bootstrap CI and significance/meaningfulness flags
- **RetrospectiveValidator**: Runs applicable QC features on real data, evaluates against ground truth, produces score matrices
- **RetrospectiveReport**: Aggregates results across datasets, checks pass criteria (p<0.05 on >=3 datasets OR effect size on >=2)
- **FeatureRunnerFactory**: 11 feature runners bridging `InjectedBatch` to F1-F12 QC analyzers, with `MockKnowledgeGraph` for KG-dependent features
- **ScenarioExecutor**: Runs scenarios, sets per-timepoint PASS/HOLD/REJECT decisions, identifies defects, generates remediations, validates against expected outcomes
- **GroundTruthValidator**: Validates detection rate (>=90%), zero false alarms, defect identification, and final decision correctness
- **ShadowProtocol**: Non-interventional PSF QC alongside standard QC with concordance/discordance tracking and PSF-unique-findings detection
- **ConcordanceAnalyzer**: Ranks features by predictive value vs functional outcomes, builds weighted composite scores, compares PSF vs standard QC
- **ThresholdCalibrator**: ROC-based threshold optimization per feature using Youden's J statistic with batch calibration and deployment API

#### Molecular-Informed Decoder PoC (`[decode]` extra)
- **BPUSimulator**: Numpy-based spike train generator for 5 simulated BPU conditions (immature, maturing, mature, stressed, drifted) with action encoding, burst injection, gamma modulation
- **SyntheticMSVGenerator**: Molecular State Vector generator with 5 canonical conditions and noise injection for measurement variability simulation
- **SpikeFeatureExtractor**: Blind (fixed regime) vs. aware (maturation-adaptive) spike feature extraction with spectral analysis (gamma/theta power), burst rate, synchrony index, Fano factor
- **MolecularBlindDecoder**: Baseline decoder using spike features only — what conventional decode pipelines use
- **MolecularAwareDecoder**: Enhanced decoder using spike features + MSV numeric vector with maturation-adaptive feature extraction (injection point [M2])
- **UniversalDecoder**: Trains on all conditions simultaneously, evaluates per-condition — most realistic scenario
- **MolecularDecodeExperiment**: Full experiment runner with ablation study (per-MSV-component) and model comparison (RF, logistic, GBM)
- **DecodeResultVisualizer**: 5 publication-quality plots (accuracy comparison, improvement bars, universal decoder, ablation heatmap, confusion matrices) + summary table
- **Design document**: `docs/molecular-informed-decoder-poc.md` — full experiment specification with expected results

#### BPU Extensions (B4)
- **OlfactoryProfiler**: Olfactory receptor expression profiling for TA2 sensor arrays — receptor diversity index, signaling cascade completeness, receptor trafficking, OMP/GAP43 maturity ratio, odorant discrimination capacity prediction
- **SpatialAnalyzer**: Spatial transcriptomics pipeline — cortical layer assignment from marker genes, per-spot pathway scoring with Moran's I spatial autocorrelation, hotspot detection, inner/outer and inter-layer regional comparison (Mann-Whitney U), laminar organisation scoring
- **`[spatial]` optional extra** for spatial transcriptomics (depends on `[sc]` for AnnData support)

#### Docker
- **API stage**: New `api` Docker build stage with FastAPI + uvicorn + pydantic, exposes port 8000
- **docker-compose.yml**: New `api` service alongside existing pipeline, dev, test, and jupyter services

#### Documentation
- **docs/api/decode.md**: Full API reference for molecular-informed decoder PoC
- **docs/api/bpu.md**: Full API reference for BPU gene sets, profiling modules, and REST API
- **docs/api/qc-validation.md**: Full API reference for L2/L3/L4 validation framework
- **docs/api/spatial.md**: Full API reference for spatial transcriptomics and olfactory profiling
- Updated: README.md, docs/how-it-works.md, docs/quickstart.md, docs/api/index.md, docs/api/bpu.md (9-layer architecture, `[decode]`/`[spatial]` extras, Docker API stage, olfactory profiler)

#### Infrastructure
- **`[bpu]`, `[api]`, `[decode]`, and `[spatial]` optional extras** in pyproject.toml
- **`l2`, `l3`, `l4` pytest markers** for selective validation test execution
- **3 shared conftest fixtures**: `qc_simulator`, `feature_runner_factory`, `dataset_registry`
- **Test count**: 1,363 → 1,706 tests (343 new)
