...
6. [Interactive HTML Report](#interactive-html-report)
7. [Validation Gates](#validation-gates)
8. [Interpretation Guardrails](#interpretation-guardrails)
9. [What NOT to Infer](#what-not-to-infer)
...
│   ├── subtype_heatmap.png               # Subtype pathway profiles heatmap
│   └── interactive_report.html           # Interactive Plotly report (optional)
├── characterization/                     # Subtype characterization exports
...
Self-contained interactive HTML report generated when `generate_interactive_report: true` is set in the config. Requires `pip install pathway-subtyping[viz]`.

### Location

`figures/interactive_report.html`

### Contents

The report includes four interactive Plotly charts:

| Chart | Description |
|-------|-------------|
| **Dimensionality Reduction Scatter** | Samples plotted in PCA, t-SNE, or UMAP space, colored by subtype |
| **Pathway Heatmap** | Mean Z-scores per subtype across all pathways |
| **Cluster Distribution** | Bar chart showing sample counts per subtype |
| **Subtype Trajectories** | Radar chart comparing pathway activation profiles across subtypes |

### How to View

- **Double-click** the HTML file to open in any modern browser (Chrome, Firefox, Safari)
- No web server or internet connection required after generation
- All JavaScript is loaded via CDN on first open (file size: ~1-5 MB)

### Interactivity

- **Hover** over data points for sample IDs and scores
- **Zoom** by dragging a selection box
- **Pan** by dragging while holding Shift
- **Download** individual charts as PNG via the Plotly toolbar
...
output:
  ...
  interactive_dim_reduction: "umap"  # "pca" (default), "tsne", or "umap"
```

### Limitations

- Requires the `[viz]` extra (plotly, umap-learn, kaleido)
- If Plotly is not installed, the pipeline silently skips report generation
- Large cohorts (>10K samples) may produce large HTML files
- UMAP requires `umap-learn` which depends on `numba` (adds ~200 MB to install)
...
*Last updated: February 2026 | Pathway Subtyping Framework v0.2.3+*
