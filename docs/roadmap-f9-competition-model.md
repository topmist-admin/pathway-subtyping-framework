...
### ⛔ Do NOT validate against `ManufacturingSimulator.inject_crosstalk`

**The only existing CROSSTALK fixture in the repo contains no shared genes**, so
validating the corrected model against it would prove nothing while appearing to
prove everything. Verified 2026-07-28:

- The simulator's gene universe is **disjoint by construction**
  (`qc/testing/simulator.py`: each pathway is assigned its own `GENE_{i}` block,
  with the index advancing per pathway and no overlap). No gene is a member of
  two pathways, so no gene can be a shared transducer.
- `inject_crosstalk` **ignores its `shared_nodes` argument entirely**, despite a
  docstring promising to honour it or synthesise shared genes. It instead takes
  the *middle* genes of pathway A and the *middle* genes of pathway B — two
  disjoint sets — and labels them shared. The code's own comment uses scare
  quotes: `# Use middle genes of each pathway as "shared nodes"`.
- What it actually injects is a +2.0 shift on A's mid-genes and a −2.5 shift on
  B's downstream genes: a two-pathway anticorrelated shift, indistinguishable
  from a stalled cascade in B plus an overdose in A.
- `metadata.parameters["n_shared_nodes"]` therefore reports a count of nodes that
  are not shared, and the existing tests assert only `defect_type == CROSSTALK`
  and a direction string, so they pass regardless.

This is the same failure shape as the score itself: an artifact that is *named*
after the mechanism without *containing* it. A competition model tested on this
fixture would be exercised against data where `S` does not exist, and the
per-observation `S(i)` in the model above would be undefined.

**Requirement.** The seven fixtures must construct genuinely shared membership —
the same gene appearing in both pathways' gene sets, with `S` computed over those
genes. Either fix `inject_crosstalk` first (make the named genes real members of
both pathways, then compete over those columns) or build the fixtures
independently of the simulator. Fixing the simulator is the better option, since
it is currently the only CROSSTALK generator and is consumed by
`qc/testing/orthogonality.py`.
...
1. All seven validation fixtures pass, and **none of them is generated by
   `inject_crosstalk` in its current form** (see the constraint above — that
   fixture contains no shared genes).
