...
Everything here uses public data only. Seed 42 throughout. No file in this directory writes to
...
|---|---|---|
| `job1_sigclust_comparator.py` | cluster-index statistic vs bootstrap-ARI, same null | ablation grid: cluster index TPR 30/30, FPR 0/30 |
| `job1b_sigclust_separation_sweep.py` | same, across the separation sweep | cluster index certifies **0.00 at every δ** |
| **`job1c_canonical_sigclust.py`** | **canonical CRAN `sigclust`**, ablation grid | **TPR 29/30, FPR 1/30**, no abstentions |
...
| **`job3b_nsweep_validated_negatives.py`** | FPR and abstention vs *n*, on **dip-validated** negatives | abstention 0.53→0.23 and false certification **0.03→0.17** across n=60/120/300 |
